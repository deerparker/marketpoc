...
export async function createSeller(
  ...
  const email = data?.email || 'seller@mercurjs.com'
  const name = data?.name || 'John Doe'
  const sellerName = data?.name || 'MercurJS Store'
...
export async function createSellerFulfillmentJourney(
  container: MedusaContainer,
  sellerId: string
) {
  const fulfillmentModule = container.resolve(FULFILLMENT_MODULE) as any

  // Create a default journey (e.g., WhatsApp for demo purposes)
