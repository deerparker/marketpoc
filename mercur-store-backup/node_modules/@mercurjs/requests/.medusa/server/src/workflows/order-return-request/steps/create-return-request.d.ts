import { CreateOrderReturnRequestDTO } from "@mercurjs/framework";
export declare const createOrderReturnRequestStep: import("@medusajs/framework/workflows-sdk").StepFunction<CreateOrderReturnRequestDTO, {
    id: string;
    customer_id: string;
    customer_note: string;
    shipping_option_id: string | null;
    vendor_reviewer_id: string | null;
    vendor_reviewer_note: string | null;
    vendor_review_date: Date | null;
    admin_reviewer_id: string | null;
    admin_reviewer_note: string | null;
    admin_review_date: Date | null;
    line_items: {
        id: string;
        line_item_id: string;
        quantity: number;
        reason_id: string | null;
        return_request: /*elided*/ any;
        created_at: Date;
        updated_at: Date;
        deleted_at: Date | null;
        return_request_id: string;
    }[];
    status: "pending" | "refunded" | "withdrawn" | "escalated" | "canceled";
    created_at: Date;
    updated_at: Date;
    deleted_at: Date | null;
} & {
    id: string;
    customer_id: string;
    customer_note: string;
    shipping_option_id: string | null;
    vendor_reviewer_id: string | null;
    vendor_reviewer_note: string | null;
    vendor_review_date: Date | null;
    admin_reviewer_id: string | null;
    admin_reviewer_note: string | null;
    admin_review_date: Date | null;
    line_items: {
        id: string;
        line_item_id: string;
        quantity: number;
        reason_id: string | null;
        return_request: /*elided*/ any;
        created_at: Date;
        updated_at: Date;
        deleted_at: Date | null;
        return_request_id: string;
    }[];
    status: "pending" | "refunded" | "withdrawn" | "escalated" | "canceled";
    created_at: Date;
    updated_at: Date;
    deleted_at: Date | null;
}[]>;
