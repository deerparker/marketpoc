...
var StatusRenderer = (value, row, column, t) => {
  ...
  const getTranslatedStatus = (status, column2) => {
    if (!t) return status;
    ...
    switch (lowerStatus) {
      ...
      default:
        if (column2.context === "payment") {
          return t(`orders.payment.status.${camelCaseStatus}`, status);
        }
        if (column2.context === "fulfillment") {
          return t(`orders.fulfillment.status.${camelCaseStatus}`, status);
        }
